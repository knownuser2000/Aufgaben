...
int main(){
    ...
    double *V_zuflus;   //wichtig für Matrix und aufgabe 4
    double *NP, *NM, *V_r;
    ...
    {   
        ...
        {
            B[8]=1;
            euler_SI(b.n,&j_t,&z_counter,b.h,B,t_1,b.tmax,b.y,z,fcn2);
        }else       //Modell 2
        {
            B[8]=0;
            euler_SI(b.n,&j_t,&z_counter,b.h,B,t_1,b.tmax,b.y,z,fcn2);
        ...
        if (y==1)
        {
            struct gnuplot_arg plot={.grid = 1,.plotTitle="Aufgabe 1 Modell 1",.length=j_t+1,.inputFile="A2_1_1.txt"};
            gnuplot(plot, legende, t_1,3, NM, NP, V_r);
        
        }else
        {
            struct gnuplot_arg plot={.grid = 1,.plotTitle="Aufgabe 1 Modell 2",.length=j_t+1,.inputFile="A2_1_2.txt"};
            gnuplot(plot, legende, t_1,3, NM, NP, V_r);        
        }

    }else if (x==2) //Aufgabe 2
    {
        printf("Geben sie die Anzahl der Zwischenschritte an:\n");
        scanf("%i", &Anzahl);
        t_1=calloc((Anzahl+1)*b.n_i+1,sizeof(double));
        z=calloc((Anzahl+1)*b.n*b.n_i+b.n,sizeof(double));

        if (y==1) //Modell 1
        {
            B[8]=1;
            euler_SI_dense_output(b.n,Anzahl,&j_t,&z_counter,b.h,B,t_1,b.tmax,b.y,z,fcn1);
        }else   //Modell 2
        {
            B[8]=0;
            euler_SI_dense_output(b.n,Anzahl,&j_t,&z_counter,b.h,B,t_1,b.tmax,b.y,z,fcn2);
        }
        ...
        if (y==1)
        {
            struct gnuplot_arg plot={.grid = 1,.plotTitle="Aufgabe 2 Modell 1",.length=j_t+1,.inputFile="A2_2_1.txt"};
            gnuplot(plot, legende, t_1,3, NM, NP, V_r);
            //gnuplot(plot, legende, t_1,1, V_r);        
        }else
        {
            struct gnuplot_arg plot={.grid = 1,.plotTitle="Aufgabe 2 Modell 2",.length=j_t+1,.inputFile="A2_2_2.txt"};
            gnuplot(plot, legende, t_1,3, NM, NP, V_r); 
            //gnuplot(plot, legende, t_1,1, V_r);        
    ...
    }else       //Aufgabe 3
    {
        ...
        {
            B[8]=1;
            euler_SI_DO_abbruch(b.n,&j_t,&z_counter,b.h,B,t_1,b.tmax,b.V_r_Max,b.y,z,fcn1);
        }else   //Modell 2
        {
            B[8]=0;
            euler_SI_DO_abbruch(b.n,&j_t,&z_counter,b.h,B,t_1,b.tmax,b.V_r_Max,b.y,z,fcn2);
        }
        //Beide Modelle brechen ab wenn sie 20m^3 erreichen
        for (int i = 0; i < b.n; i++)
        {
            printf("y[%i]=%lf\n",i,b.y[i]);
        }
        ...
        //und wechseln die funktion
        if (y==1)   //Modell 1
        {
            euler_SI(b.n,&j_t,&z_counter,b.h,B,t_1,b.tmax,b.y,z,fcn3);
        }else       //Modell 2
        {
            euler_SI_DAE_In_2(b.n,&j_t,&z_counter,b.h,B,t_1,b.tmax,b.V_r_Max,b.y,z,fcn4);
            //euler_SI(b.n,&j_t,&z_counter,b.h,B,t_1,b.tmax,b.y,z,fcn2);
        }
        Z_=calloc((j_t+z_counter),sizeof(double));
        ...
        if (y==1)
        {
        struct gnuplot_arg plot={.grid = 1,.plotTitle="Aufgabe 3 Modell 1",.length=j_t+1,.inputFile="A2_3_1.txt"};
        gnuplot_multi_y(plot,legende,legende1,t_1,3,1,NM,NP,V_r,V_zuflus);        
        }else
        {
        struct gnuplot_arg plot={.grid = 1,.plotTitle="Aufgabe 3 Modell 1",.length=j_t+1,.inputFile="A2_3_2.txt"};
        gnuplot_multi_y(plot,legende,legende1,t_1,3,1,NM,NP,V_r,V_zuflus);
        }  */
    }
    ...
    */
    ...
    z_counter_2= z_counter;
    B[8]=0;
    euler_SI(b.n,&j_t,&z_counter,b.h,B,t_1,b.tmax,b.y,z,fcn4);
